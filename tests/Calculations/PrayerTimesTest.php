...
class PrayerTimesTest extends TestCase
{
     ...
     * Using ISNA method
     */
    public function testPrayerTimesCalculationForBothellWA20260101()
    {
        ...
        // Create PrayerTimes instance with ISNA method
        $pt = new PrayerTimes(Method::METHOD_ISNA);
     ...
     */
    public function testPrayerTimesCalculationForBothellWA20260625Summer()
    {
        // Bothell, WA coordinates
        $latitude = 47.7580361;
        $longitude = -122.1985255;
        ...
        $times = $pt->getTimes(
            ...
            $longitude,
            null,
            PrayerTimes::LATITUDE_ADJUSTMENT_METHOD_MOTN, // NightMiddle to match times.js default
            null,
            PrayerTimes::TIME_FORMAT_24H
        );
    ...
    private function assertEqualsWithTolerance($expected, $actual, $toleranceMinutes, $message)
    {
        ...
        $this->assertLessThanOrEqual(
            ...
            $diff,
            sprintf(
                '%s should match within %d minutes. Expected: %s, Got: %s (diff: %.1f min)',
                $message,
                $toleranceMinutes,
                $expected,
                $actual,
